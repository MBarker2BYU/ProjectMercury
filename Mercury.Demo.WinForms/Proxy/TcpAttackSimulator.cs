...
public sealed class TcpAttackSimulator(int listenPort, int targetPort) : IAsyncDisposable
{
    ...
    private async Task AcceptLoopAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                Console.WriteLine(@$"[AttackSimulator] Accept error: {exception.Message}");
            }
        }
    }
    ...
    private async Task HandleClientAsync(TcpClient client, CancellationToken cancellationToken)
    {
        ...
        using (var server = new TcpClient())
        {
            ...
            {
                Console.WriteLine(@$"[AttackSimulator] Connection ended: {exception.Message}");
            }
            catch (SocketException exception)
            {
                Console.WriteLine(@$"[AttackSimulator] Socket error: {exception.Message}");
            }
            catch (Exception exception)
            {
                Console.WriteLine(@$"[AttackSimulator] Proxy error: {exception.Message}");
            }
        }
    }
    ...
    private byte[] ApplyAttack(byte[] frame)
    {
        ...
        if (replayFrame != null)
        {
            Console.WriteLine(@"[AttackSimulator] REPLAY - sending previous complete frame");
        ...
        Console.WriteLine(@$"[AttackSimulator] TAMPER applied to protected payload (len={tamperedFrame.Length})");
