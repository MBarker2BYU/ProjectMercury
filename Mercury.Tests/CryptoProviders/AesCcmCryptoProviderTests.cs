...
// Author           : Matthew D. Barker / Kim k. Brown
// Created          : 07-17-2026
//
...
/// <summary>
/// Class AesCcmCryptoProviderTests. This class cannot be inherited.
/// </summary>
public sealed class AesCcmCryptoProviderTests : CryptoProviderContractTests
{
    /// <summary>
    ...
    [InlineData(1024)]
    [InlineData(4096)]
    public async Task SealAndOpenAsync_VariousPayloadLengths_RoundTripsWithExpectedLayout(int payloadLength)
    {

        var provider = CreateProvider(MercuryTestFactory.BuildKeyProvider());
        var payload = MercuryTestFactory.CreatePayload(payloadLength);

        var sealRequest = new TestSealRequest(MercuryTestFactory.BuildContext(),
            new MercuryMemory(payload));

        var sealedResult = await provider.SealAsync(sealRequest, EnvelopeService.Instance);
        ...
        Assert.Equal(payloadLength + 1 + 12 + 16, protectedPayload.Length);

        var openRequest = new TestOpenRequest(sealedResult.ValidatedEnvelope);
        var openedResult = await provider.OpenAsync(openRequest, EnvelopeService.Instance);
    ...
    [Fact]
    public async Task SealAsync_SamePayloadTwice_GeneratesDistinctNonces()
    {
        ...
        var second = await provider.SealAsync(request, EnvelopeService.Instance);
        ...
        var firstNonce = firstProtectedPayload.Skip(1).Take(nonceLength).ToArray();
        var secondNonce = secondProtectedPayload.Skip(1).Take(nonceLength).ToArray();
