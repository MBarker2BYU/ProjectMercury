The Mercury Project
Release: Mercury RC 1.2  
Version: 1.2.0-rc.1  
Status: CSE499 Release Candidate
Team Quotes
...
Demonstrate a secure communications and data exchange framework.
Protect payload confidentiality.
Detect tampering before unprotected data is released.
Validate protected communications using authenticated encryption.
Reject replayed protected communications.
Separate cryptographic logic from transport logic.
Support interchangeable transport providers.
Support interchangeable cryptographic providers.
Provide a developer-facing API that encourages safe usage.
Fail safely when protected communications are invalid, corrupted, replayed, or unauthorized.
Provide working demos that prove the core security properties.
Provide automated tests that validate core framework behavior.
---
Academic Scope
...
The selected cryptographic algorithm is implemented correctly by the underlying platform.
Keys are generated with sufficient entropy.
Keys are stored and provided securely outside the current framework scope.
Sender and recipient key identifiers are assigned consistently.
Replay-protection state is available for the lifetime of the receiving context.
The application treats failed receive results as rejected communications.
The transport may be hostile, unreliable, or observable.
Attackers do not already possess the correct secret key material.
---
...
Protect data before transport.
Treat transport as untrusted.
Authenticate protected data before releasing unprotected data.
Reject replayed protected communications.
Keep security logic separate from delivery logic.
Fail safely.
Avoid overclaiming security properties not implemented by the current version.
---
Architecture Overview
...
Send and receive a protected text payload.
Select AES-GCM or ChaCha20-Poly1305.
Select Binary or JSON envelope encoding.
Use supported transports available in the selected demo.
Capture and replay a previously accepted protected frame.
Modify a protected frame in transit.
Confirm that replayed or tampered communications are rejected.
Confirm that unprotected payload data is released only after successful validation.
